...
from fca.ai_client import AIClient
from fca.db import Database, Repos, SCHEMA_VERSION
from fca.dictionary import build_dictionary
from fca.srs import SRSState
from fca.transfer import CSV_FIELDS, export_csv, export_pack, import_csv, import_pack
...
def test_search_uses_accent_and_apostrophe_equivalence(repos):
    assert any(w["target"] == "école" for w in repos.words.search("ecole"))
    assert any(w["target"] == "garçon" for w in repos.words.search("garcon"))
    assert any(w["target"] == "lait" for w in repos.words.search("süt"))
...
def test_csv_roundtrip_preserves_unicode(repos, tmp_path):
    path = tmp_path / "mots-accentués.csv"; export_csv(repos.words.search("école"), path)
    text = path.read_text(encoding="utf-8-sig"); assert "école" in text
    other = Repos(Database(tmp_path / "import.db")); before = other.words.count(); import_csv(other.words, path)
    assert any(w["target"] == "école" for w in other.words.search("ecole")); other.db.close()
...
def test_dict_entries_gets_source_and_example_columns_on_old_databases(tmp_path):
    ...
    old.execute("INSERT INTO dict_entries(headword,translation,pos,extra,note) VALUES('bretzel','pretzel','n','m','')")
    ...
    assert [(r["headword"], r["source"], r["example"]) for r in rows] == [("bretzel", "user", "")]
    assert repos.dictionary.add("chouquette", "choux bun", "n", "f", "", "ai", "Une chouquette, s'il vous plaît.") == 1
    assert repos.dictionary.add_many([("bretzel", "pretzel")], "ai") == 0            # UNIQUE(headword, translation) still holds
    ...
    chouquette, bretzel = d.lookup("chouquette")[1][0], d.lookup("bretzel")[1][0]
    assert (chouquette.source, chouquette.example, bretzel.source) == ("ai", "Une chouquette, s'il vous plaît.", "user")
