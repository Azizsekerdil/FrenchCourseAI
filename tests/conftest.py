...
from fca import config as C  # noqa: E402  (must come after the home override)
...
class MockOpenAI:
    ...
    ``require_key``: when set, every request without ``Authorization: Bearer <require_key>``
    is answered 401 (a keyed cloud API); empty = key-less server (LM Studio / Ollama).
    """
    MODEL = "mock-model"
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _authorized(self) -> bool:
                if server.require_key and self.headers.get("Authorization", "") != f"Bearer {server.require_key}":
                    self._send({"error": {"message": "Unauthorized", "code": 401}}, 401); return False
                return True
