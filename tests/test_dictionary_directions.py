...
from fca.db import Database, Repos
...
SAMPLE = {
    ...
    "fr": [("maison", "n", "f", "house; home", "ev; hane"), ("arbre", "n", "m", "tree", "ağaç"),
           ("chaise", "n", "f", "chair", ""), ("aller", "v", "", "to go", "gitmek; yürümek")],
...
def test_normalize_search_folds_turkish_letters():
    """Search strips diacritics (ç/c, ğ/g, ş/s, ö/o, ü/u) and folds dotless ı to i, just as the capital I lowers to i."""
    assert C.normalize_search("çğışöü") == "cgisou" and C.normalize_search("Çiçek") == C.normalize_search("çiçek") == "cicek"
    assert C.normalize_search("İyi") == C.normalize_search("iyi") == "iyi"
    assert C.normalize_search("Işık") == C.normalize_search("ışık") == C.normalize_search("IŞIK") == "isik"   # capital I lowers to i: ı/i are search-equivalent
    assert C.normalize_search("ağaç") == C.normalize_search("agac") == "agac" and C.normalize_search("Şeker") == "seker" and C.normalize_search("gitmek") == "gitmek"
...
def test_extend_merges_a_turkish_gloss_into_an_existing_twin():
    ...
    assert dic.extend([D.Entry(HOUSE, "n", "", SAMPLE[0][3], "", D.SOURCE_AI, "", "hane")]) == 0 and dic.twin(ai).tr == twin.tr   # never overwritten
...
def test_twin_folds_the_part_of_speech_and_treats_an_unknown_one_as_wildcard():
    """The Add-entry dialog stores pos "" and CSV rows say "noun"; the AI answers "n". SQLite keys rows by (headword, translation)
    alone, so the in-memory dictionary must resolve the same twin - otherwise the AI gloss lands next to the entry, not in it."""
    assert D.fold_pos("") == "" and D.fold_pos("noun") == "n" and D.fold_pos("Verb") == "v" and D.fold_pos("adj") == "adj"
    assert D.pos_compatible("", "n") and D.pos_compatible("noun", "n") and D.pos_compatible("n", "") and not D.pos_compatible("adj", "n")
    assert D.sense_keys("to fly; to soar") == {"to fly", "fly", "to soar", "soar"} and D.sense_keys("attic; loft") & D.sense_keys("attic")
    assert not D.sense_keys("to steal; to rob") & D.sense_keys("to fly")
    bare, noun = D.Entry("Qqzzpluie", "", "", "rain", "", D.SOURCE_USER), D.Entry("Qqzzneige", "noun", "f", "snow", "", D.SOURCE_USER)
    dic = D.Dictionary([bare, noun])
    ai_rain = D.Entry("Qqzzpluie", "n", "f", "rain", "", D.SOURCE_AI, "", "yağmur")
    ai_snow = D.Entry("Qqzzneige", "n", "f", "snow", "", D.SOURCE_AI, "", "kar")
    assert dic.twin(ai_rain) is bare and dic.twin(ai_snow) is noun and dic.contains(ai_rain) and dic.contains(ai_snow)
    assert dic.twin(D.Entry("Qqzzpluie", "v", "", "rain")) is bare                      # unknown stored pos matches any
    assert dic.twin(D.Entry("Qqzzneige", "", "", "snow")) is noun                        # unknown incoming pos matches any
    assert dic.twin(D.Entry("Qqzzneige", "v", "", "snow")) is None                       # two known, different parts of speech: distinct entries
    assert dic.find("Qqzzneige", "n") == [noun] and dic.find("Qqzzneige", "noun") == [noun] and dic.find("Qqzzpluie", "n") == []
    assert dic.extend([ai_rain, ai_snow]) == 0 and len(dic) == 2                        # merged, not added…
    assert [e.pos for e in dic.entries] == ["", "noun"] and [e.tr for e in dic.entries] == ["yağmur", "kar"]   # …keeping their own pos
    # the one built-in pair that shares headword and translation across parts of speech stays two entries
    dic = D.Dictionary([D.Entry("orange", "n", "f", "orange"), D.Entry("orange", "adj", "", "orange")])
    assert len(dic) == 2 and dic.twin(D.Entry("orange", "adj", "", "orange")).pos == "adj" and dic.twin(D.Entry("orange", "", "", "orange")).pos == "n"
    assert len(D.build_dictionary()) == len(D.builtin_entries())                         # no built-in line is swallowed by the fold
...
def test_parse_ai_entries_accepts_translation_en_tr_and_legacy_translation():
    rows = D.parse_ai_entries(json.dumps([
        ...
        {"headword": "Ccc", "pos": "n", "extra": "", "translation": "old", "translation_en": "new", "translation_tr": ["ağaç", "orman"]},
    ...
    if D.HAS_TR:
        ...
        assert by["Ccc"].tr == "ağaç; orman" and "Ddd" not in by                         # English is required
...
def test_dict_entries_gets_a_tr_column_on_old_databases(tmp_path):
    ...
    assert repos.dictionary.add_many([("Semmel", "bread roll", "n", "", "", "", "sandviç ekmeği")]) == 0     # UNIQUE(headword, translation) still holds
    assert repos.dictionary.count() == 3
    by = {r["headword"]: r["tr"] for r in repos.dictionary.all()}
    assert by == {"Brezel": "simit", "Kipferl": "ay çöreği", "Semmel": ""}
