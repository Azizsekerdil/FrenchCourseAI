...
MACOS_VOICE = "Thomas"                 # voice used by the macOS `say` command


class Speaker:
    def __init__(self, rate: int = 155): self.rate = rate
    def speak(self, text: str) -> None:
        if text: threading.Thread(target=self._run, args=(text,), daemon=True).start()
    def _run(self, text: str) -> None:
        ...
        script = ("Add-Type -AssemblyName System.Speech; $s=New-Object System.Speech.Synthesis.SpeechSynthesizer; "
                  ...
                  "if($v){$s.SelectVoice($v.VoiceInfo.Name)}; " + f"$s.Speak('{escaped}')")
        try:
    ...
    def info(self):
        ...
        return "Windows System.Speech (fr-FR when installed)"
