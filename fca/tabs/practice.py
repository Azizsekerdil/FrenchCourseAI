...
class AITab(BaseTab):
    ...
    def send(self):
        ...
        if task == "vision": prompt = prompt or "Read the French text in this image, transcribe it, translate it, and explain important accents, pronunciation or grammar."
...
class SpeakingTab(BaseTab):
    ...
    def build(self):
        self.heading(); main = self.card(); main.pack(fill="both", expand=True)
        bar = ttk.Frame(main, style="Card.TFrame"); bar.pack(fill="x")
        self.scenario = tk.StringVar(value="Au café")
        ttk.Label(bar, text=self.t("speak.scenario"), style="CardMuted.TLabel").pack(side="left")
        ttk.Combobox(bar, textvariable=self.scenario, state="readonly", values=("Au café", "À la gare", "À l'hôtel", "Dans un magasin", "Chez le médecin"), width=24).pack(side="left", padx=8)
    ...
    def begin(self):
        ...
        prompt = f"Start a short A1 French role-play in the scenario '{self.scenario.get()}'. Speak as the other person. Ask only one short question."
...
class WritingTab(BaseTab):
    ...
    def correct(self):
        prompt = "Correct this French learner text. Show a corrected version, name the grammar rule, and give a brief explanation including accents and elision when relevant:\n\n" + self.input.get("1.0", "end-1c")
