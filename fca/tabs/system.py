...
class SettingsTab(BaseTab):
    ...
    def on_show(self):
        """Called by the shell whenever the page is selected: the form must show the live settings, not the values from build time
        (``dict_ai`` is also edited from the dictionary toolbar; a stale form would write the old value back on Save)."""
        s = self.app.settings
        self.theme.set(s["theme"]); self.goal.set(int(s["daily_goal"])); self.tts.set(bool(s["tts_enabled"])); self.ai.set(bool(s["ai_enabled"]))
    ...
    def test_alt(self):
        ...
        def done(models):
            ...
            found = "" if not model or model in models else f" · {self.t('settings.alt_model_missing')}"
            self.alt_result.configure(text=f"{self.t('settings.alt_ok')} · {len(models)} {self.t('settings.alt_models')}{found}")
